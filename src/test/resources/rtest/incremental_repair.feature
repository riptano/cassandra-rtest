...
Feature: Incremental Repair
  ...
  Scenario Outline: Complete incremental repair without triggering compaction
    ...
    And within 10 minutes I cannot find any data in "repair_quality" keyspace showing a pending repair
    And a "incremental" repair would not find out-of-sync "<tokens>" ranges for keyspace "repair_quality"
    Examples:
      | parallelism | tokens |
      |    parallel |    all |
